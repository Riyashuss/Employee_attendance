...
<?php $__env->startPush('js'); ?>
    ...
    function Approved(workOrderId, status_code) {
        ...
        if (status_code=="413")
       {
         Swal.fire({
                ...
                text: '<?php echo app('translator')->get('words.pleaseconfirmtoapprovemodelcreation'); ?>',
    ...
    //    {
		...
		if($.trim(remarks) == "")
		{
         ...
         Swal.fire({
                icon: 'warning',
                title: '<?php echo app('translator')->get('words.workorder'); ?>',
                text: '<?php echo app('translator')->get('words.pleaseconfirmtorejectmodelcreation'); ?>',
                showCancelButton: true,
                customClass: {
                    confirmButton: 'btn btn-success mx-2',
                    cancelButton: 'btn btn-danger mx-2'
    ...
    $.ajax({
        url: '<?php echo e(route('modelapproveWorkOrder')); ?>',
        type: 'post',
            data:fd,
            contentType: false,
            processData: false,
            success: function(response){
                ...
                if (response.success)
                {
                    Swal.fire({
                                icon: 'success',
                                title: '<?php echo app('translator')->get('words.workorder'); ?>',
                                text: '<?php echo app('translator')->get('words.modelcreationapprovedsuccessfully'); ?>',
                                ...
                                window.location.href = "/home_modelqcassign";
                              });
                }
        },
...
function rejectedWorkOrder(workOrderId) {
    ...
    // Make an AJAX request
    $.ajax({
        url: '<?php echo e(route('modelrejectedWorkOrder')); ?>',
        type: 'post',
            data:fd,
            contentType: false,
            processData: false,
            success: function(response){
                ...
                if (response.success)
                {
                    Swal.fire({
                                icon: 'success',
                                title: '<?php echo app('translator')->get('words.workorder'); ?>',
                                text: '<?php echo app('translator')->get('words.modelcreationrejectedsuccessfully'); ?>',
                                showCancelButton: false,
                                customClass: {
                                    confirmButton: 'btn btn-success mx-2'
                                },
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\Dk3app_Final\Dk3app_Final\dk3app\resources\views/viewqcmodel.blade.php ENDPATH**/ ?>