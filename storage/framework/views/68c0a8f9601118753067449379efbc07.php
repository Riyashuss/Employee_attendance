			...
			<?php echo csrf_field(); ?>
						<div class="card card-custom">
									...
									<?php
										...
										$leaveStatus = \App\Models\Attendance::where('user_id', $userId)
											->whereDate('leave_date', $currentDate)
											->value('leave_id'); // Assuming leave_id indicates leave status (0 = not on leave, 1 = on leave)
									?>

									<?php if($leaveStatus == 0): ?>
										<!-- Show the card body when the user is not on leave -->
											...
											<div class="row mt-2">
												...
												<div class="col-lg-6">
													<div class="input-group">
														<input id="inWorkOrderName" name="inWorkOrderName" class="form-control rounded" type="text" readonly value="<?php echo e(auth()->user()->username); ?>"/>
													</div>
...
unset($__errorArgs, $__bag); ?>
													...
													<div class="input-group">
														<?php
															$existingAttendance = \App\Models\Attendance::where('user_id', $userId)
																...
																->select('attendance.location_id', 'location.location_name')
																->first();
														?>

														<?php if($existingAttendance == null): ?>
															<select class="form-control" name="location" id="location" onchange="showCheckInButton()">
														...
														<?php else: ?>
															<input type="hidden" name="location" value="<?php echo e($existingAttendance->location_id); ?>"/>
															<input id="location_display" name="location_display" class="form-control rounded" type="text" readonly value="<?php echo e($existingAttendance->location_name); ?>"/>
														<?php endif; ?>
													</div>
												</div>
											</div>
											...
											<div class="row mt-4">
												<div class="col-lg-10 d-flex justify-content-center">
													...
													<?php if(!$existingAttendance || !$existingAttendance->checkout): ?>
														<button 
															id="btnCheckInOut" 
															onclick="toggleCheckInOut()" 
															type="button" 
															class="btn <?php echo e($existingAttendance && !$existingAttendance->checkout ? 'btn-danger' : 'btn-success'); ?>"
															data-status="<?php echo e($existingAttendance && !$existingAttendance->checkout ? 'checkout' : 'checkin'); ?>">
															<?php echo e($existingAttendance && !$existingAttendance->checkout ? 'Check Out' : 'Check In'); ?>

														</button>
													<?php else: ?>
														<!-- Optional message if already checked in and checked out -->
														<p class="text-center text-success">You have checked in and checked out for today.</p>
													<?php endif; ?>
												</div>
											</div>
	
												<div class="row mt-1"> 
													<div class="col-lg-8 pt-1 text-end">
													<label class="form-label">
														Current Time: <span id="runningTime"></span>
													</label>
													</div>
												</div>
											
												
												<div class="col-lg-12 pt-1  text-end">
													<?php
														$userId = auth()->user()->id;

														// Retrieve the attendance record for today
														$attendance = \App\Models\Attendance::where('user_id', $userId)
															->whereDate('checkin', Carbon::today()) // Ensure we only fetch today's attendance
															->first(); // Get the first record or null if no record found
													?>

													<div class="input-group">
														<!-- Check if the attendance exists and display checkin time -->
														<label class="form-label col-lg-8 pt-1  text-end"style="color: green;">
															<?php if($attendance): ?>
																checkin:<?php echo e($attendance->checkin); ?>
...
unset($__errorArgs, $__bag); ?>
												...
												<div class="col-lg-12 pt-1  text-end">
													<?php
														...
														// Retrieve the attendance record for today
														$attendance = \App\Models\Attendance::where('user_id', $userId)
															->whereDate('checkout', Carbon::today()) // Ensure we only fetch today's attendance
															->first(); // Get the first record or null if no record found
													?>

													<div class="input-group">
														<!-- Check if the attendance exists and display checkin time -->
														<label class="form-label col-lg-8 pt-1  text-end"style="color: red;">
															<?php if($attendance): ?>
																checkout:<?php echo e($attendance->checkout); ?>
...
unset($__errorArgs, $__bag); ?>
											...
											<div class="row mt-5">
												<div class="col-lg-10 d-flex justify-content-center">
													<!-- Optional: Display a message when the user is on leave -->
													<p class="text-danger text-center">You are on leave today.</p>
													<!-- Hide the check-in/out sections -->
													<style>
														#cardBodyMain, #btnCheckInOut, #location, #inWorkOrderName {
															display: none;
														}
													</style>
												</div>
...
</html><?php /**PATH C:\xampp\htdocs\Employee_attendance\resources\views/home_attendance.blade.php ENDPATH**/ ?>