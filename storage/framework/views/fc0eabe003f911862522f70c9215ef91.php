...
<div class="ps__rail-x" style="left: 0px; bottom: 0px;"><div class="ps__thumb-x" tabindex="0" style="left: 0px; width: 0px;"></div></div><div class="ps__rail-y" style="top: 0px; right: 0px;"><div class="ps__thumb-y" tabindex="0" style="top: 0px; height: 0px;"></div></div></aside>
    ...
    <div class="card-body">
    <div class="">
        <div class="card" id="basic-info" style="width:60%; margin: auto;">
        <h5 class="mb-4 text-center">User Details</h5>

<?php
    use Carbon\Carbon;
?>

<?php $__currentLoopData = $groupedProjects; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $projectName => $users): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <div class="project-section p-3 bg-white text-dark" style="border-radius: 10px;">


        <!-- Absent Users -->
        <p><strong>Absent Users:</strong></p>
        <?php $__currentLoopData = $users; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php if($user->leave_id == 1 && Carbon::parse($user->leave_date)->isToday()): ?> <!-- Leave today (absent) -->
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div class="d-flex align-items-center gap-2 w-50">
                        <p><strong></strong> <?php echo e($user->username ?? 'No user assigned'); ?></p>
                    </div>
                    <div>
                        <button type="button"
                                class="btn btn-danger"
                                data-bs-toggle="modal"
                                data-bs-target="#viewModal-<?php echo e($user->user_id); ?>">
                            View
                        </button>
                    </div>
                </div>
                    ...
                    <!-- Modal for each user -->
            <div class="modal fade" id="viewModal-<?php echo e($user->user_id); ?>" tabindex="-1" aria-labelledby="viewModalLabel-<?php echo e($user->user_id); ?>" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="viewModalLabel-<?php echo e($user->user_id); ?>">User Details</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <p><strong>User ID:</strong> <?php echo e($user->user_id ?? 'No ID assigned'); ?></p>
                            <p><strong>Username:</strong> <?php echo e($user->username ?? 'No user assigned'); ?></p>
                            <p><strong>Present Count (This Month):</strong> <?php echo e($userStats[$user->user_id]->present_count ?? 0); ?></p>
                            <p><strong>Absent Count (This Month):</strong> <?php echo e($userStats[$user->user_id]->absent_count ?? 0); ?></p>
                            <p><strong>Total Days (This Month):</strong> <?php echo e($userStats[$user->user_id]->total_days ?? 0); ?></p>
                        </div>
                        <div class="modal-footer">
        ...
        <p><strong>Present Users:</strong></p>
        <?php $__currentLoopData = $attendance; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <?php if($user->leave_id == 0 && Carbon::parse($user->leave_date)->isToday()): ?>
            <div class="d-flex justify-content-between align-items-center mb-3">
                <div class="d-flex align-items-center gap-2 w-50">
                    <p>
                        
                        <strong></strong> <?php echo e($user->username ?? 'No user assigned'); ?>
        ...
        <p><strong>User not informed:</strong></p>
        <?php $__currentLoopData = $allUsers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $allUser): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php
                ...
                $isAbsent = $users->contains(function ($user) use ($allUser) {
                    return $user->user_id == $allUser->id && $user->leave_id == 1 && Carbon::parse($user->leave_date)->isToday();
                });

                // Check if the user is present
                $isPresent = $users->contains(function ($user) use ($allUser) {
                    return $user->user_id == $allUser->id && $user->leave_id == 0 && Carbon::parse($user->leave_date)->isToday();
                });
            ?>

            <?php if(!$isAbsent && !$isPresent): ?>
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div class="d-flex align-items-center gap-2 w-50">
                        <p>
                            
                            <strong></strong> <?php echo e($allUser->username ?? 'No user assigned'); ?>
                    ...
                    <div>
                        <button type="button"
                            class="btn btn-danger"
                            data-bs-toggle="modal"
                            data-bs-target="#viewModal-<?php echo e($allUser->id); ?>">
                        View
                    </button>
                    </div>
                </div>
                 <!-- Modal for each user -->
            <div class="modal fade" id="viewModal-<?php echo e($allUser->id); ?>" tabindex="-1" aria-labelledby="viewModalLabel-<?php echo e($allUser->id); ?>" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="viewModalLabel-<?php echo e($allUser->id); ?>">User Details</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <p><strong>User ID:</strong> <?php echo e($allUser->id ?? 'No ID assigned'); ?></p>
                            <p><strong>Username:</strong> <?php echo e($allUser->username ?? 'No user assigned'); ?></p>
                            <p><strong>Present Count (This Month):</strong> <?php echo e($userStats[$allUser->id]->present_count ?? 0); ?></p>
                            <p><strong>Absent Count (This Month):</strong> <?php echo e($userStats[$allUser->id]->absent_count ?? 0); ?></p>
                            <p><strong>Total Days (This Month):</strong> <?php echo e($userStats[$allUser->id]->total_days ?? 0); ?></p>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                        </div>
...
<?php /**PATH C:\xampp\htdocs\Employee_attendance\resources\views/laravel/project/user_project.blade.php ENDPATH**/ ?>