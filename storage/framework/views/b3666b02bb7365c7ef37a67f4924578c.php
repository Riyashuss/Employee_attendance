...
<?php $__env->startSection('content'); ?>
    ...
    <div class="card mx-1 my-1  mt-1 pt-1" id="basic-info">
		<div class="card-header  mt-1 pt-1">
			<h5>Work Order assignment to Model Creation</h5>
		</div>
...
<?php $__env->startPush('js'); ?>
	...
	function processUpload()
	{
		...
		{
			$('#file1_error').html("Please select a Revit file");
			$('#file1_error').show();
			isError = true;
			
		}
		else if ($('#file1')[0].files[0].name.toLowerCase() != workOrderName.toLowerCase() + "_3d_modeling.dwg")
		{
			console.log($('#file1')[0].files[0].name.toLowerCase());
			$('#file1_error').html("Invalid File. File name should end with '" + workOrderName + "_3D_Modeling.dwg'");
			$('#file1_error').show();
			isError = true;
		}
		...
		if (!isError)
		{
			...
			ModelingName = $('#file1')[0].files[0].name;	
			uploadBaseMapDwg();
	...
	function uploadBaseMapDwg()
	{
		...
		var fd = new FormData();
			...
			$.ajax({
				...
				url: '<?php echo e(route('file.upload.dwgForModel')); ?>',
				type: 'post',
				data:fd,
				contentType: false,
				processData: false,
				success: function(response){
					...
					if (response.success)
					{
						ModelingNameServer = response.fileNameServer;
						processAssign();
					}
	...
	function processAssign()
	{
			...
			fd.append('ModelingName',ModelingName);
			fd.append('ModelingNameServer',ModelingNameServer);
			
			fd.append('_token','<?php echo e(csrf_token()); ?>');
			
			$.ajax({
				 
				url: '<?php echo e(route('assignToModelPost')); ?>',
				type: 'post',
				data:fd,
				contentType: false,
				processData: false,
				success: function(response){
					...
					if (response.success)
					{
						Swal.fire({
									...
									text: "WorkOrder is assigned to Model-Creation",
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/dk3app/resources/views/laravel/modelcreation/assigntomodelcreation.blade.php ENDPATH**/ ?>