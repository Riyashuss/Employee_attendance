    ...
    <a class="navbar-brand"style="backgrounds-color:white" href="">
        ...
        <ul class="navbar-nav">
            ...
            <li class="nav-item" id="ddd">
                <a href="http://127.0.0.1:8000/home_user" class="nav-link " aria-controls="dashboard" role="button" aria-expanded="false">
                    <div class="icon icon-shape icon-sm text-center d-flex align-items-center justify-content-center">
                        <!-- <i class="fa fa-house text-sm opacity-10" style="color: #FD1717;"></i> -->
                    </div>
            ...
            <li class="nav-item sidemenuitem" id="menuAttendance" style="">
                <a href="http://127.0.0.1:8000/home_attendance" class="nav-link " aria-controls="attendance" role="button" aria-expanded="false">
                    <div class="icon icon-shape icon-sm text-center d-flex align-items-end justify-content-center">
            ...
            <li class="nav-item sidemenuitem" id="menuProject" style="">
                <a href="http://127.0.0.1:8000/home_Project" class="nav-link " aria-controls="Project" role="button" aria-expanded="false">
                    <div class="icon icon-shape icon-sm text-center d-flex align-items-end justify-content-center">
            ...
            <li class="nav-item sidemenuitem" id="details" style="display:none;">
                ...
                <div class="collapse show" id="details">
                    <ul class="nav ms-4">
                        <li class="nav-item ">
                            <a class="nav-link " href="http://127.0.0.1:8000/user_details">
                                <div class="icon icon-shape icon-sm text-center d-flex align-items-end justify-content-center">
                ...
                <div class="collapse show" id="details">
                    <ul class="nav ms-4">
                        <li class="nav-item ">
                            <a class="nav-link " href="http://127.0.0.1:8000/projectview">
                                <div class="icon icon-shape icon-sm text-center d-flex align-items-end justify-content-center">
                                    <!-- <i class="fa fa-file-pen text-sm opacity-10" style="color: #00FFFF;"></i> -->
                                </div>
...
<!-- jQuery (for Bootstrap 4 support) -->
...
<!-- Bootstrap JS (use the correct version for your project, 4.x or 5.x) -->
...
<script>
    // Bootstrap Modal Event for dynamically injecting content
    $('#taskModal').on('show.bs.modal', function (event) {
        var button = $(event.relatedTarget); // Button that triggered the modal
        var projectName = button.data('project-name'); // Extract project name from data-* attributes
        var modal = $(this);

        // Clear any previous task content
        modal.find('#modal-task-content').empty();

        // Loop through projects to display tasks for the selected project
        var tasksHtml = '';
        <?php $__currentLoopData = $projects; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $project): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            if ('<?php echo e($project->project_name); ?>' === projectName) {
                tasksHtml += '<p><strong>Task: </strong><?php echo e($project->task_name); ?></p>';
            }
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>

        // Insert tasks into modal content
        modal.find('#modal-task-content').html(tasksHtml);
    });
</script>
<div class="card-body" >
   <div class="container">
       <h3 class="mb-4 text-center">Project Details</h3>
       <div class="card" id="basic-info"style="max-width: 80%; margin:auto; padding: 10px;">
           
               <?php $lastProjectName = null; ?>
               <?php $__currentLoopData = $projects; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $project): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                   <?php if($lastProjectName !== $project->project_name): ?>
                       <?php if($lastProjectName !== null): ?>
                           </div> <!-- Close the previous project section -->
                       <?php endif; ?>
                       <div class="p-3 bg-light text-dark mb-3 project-item" 
                            style="border-radius: 10px; margin:auto; width:100%;">
                           <div class="d-flex justify-content-between align-items-center flex-wrap">
                               <p class="mb-1"><strong>Project: </strong><?php echo e($project->project_name); ?></p>
                       ...
                       <?php $lastProjectName = $project->project_name; ?>
                   <?php endif; ?>
               <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
           </div>
       </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="POST" action="<?php echo e(route('task.update')); ?>">
                    <?php echo csrf_field(); ?>
                    <div class="form-group">
                        <input type="hidden" name="task_id" value="<?php echo e($task->id ?? ''); ?>">
                        <input type="hidden" name="task_name" id="task-name" value="">

                        <label for="task-duration">Task Duration</label>
                        <input type="text" class="form-control" id="task-duration" name="duration">
                    </div>
...
<script>
    $(document).on('click', '.view-tasks-btn', function () {
        ...
        $('.view-tasks-btn').css('background-color', 'green');
...
<div class="task-row" style="display: flex; align-items: center; margin: 2px;">
    <div class="task" style=" min-width:50%;text-align: left;">
        <strong>Task:</strong><?php echo e($project->task_name); ?>

    </div>
    <div class="status" style="min-width: 50%; text-align: center;">
        <strong>Status:</strong> <?php echo e($project->status == 2 ? 'Completed' : ($project->status == 1 ? 'In Progress' : 'Not Started')); ?>
                ...
                <?php if($project->status == 1): ?>
                    <!-- Show 'Update' button when status is 1 -->
                    <button class="btn btn-sm btn-info"
                            data-toggle="modal"
                            data-target="#updateTaskModal"
                            data-task-name="<?php echo e($project->task_name); ?>">
                        Update
                    </button>
                <?php elseif($project->status == 2): ?>
                    <!-- Show 'Completed' button with disabled when status is 2 -->
                    <button class="btn btn-sm btn-success" disabled>
                        Completed
                    </button>
                <?php endif; ?>
                <!-- No button for status = 0 -->
            </div>
        `;
    }
<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    ...
    $(document).on('click', '[data-target="#updateTaskModal"], [data-target="#completeTaskModal"]', function () {
        var taskName = $(this).data('task-name');

        // Set the task name in the respective modal's input field
        var targetModal = $(this).data('target');
        $(targetModal).find('#task-name').val(taskName);
    });


    // Inject tasks into the modal
    modal.find('#modal-task-content').html(tasksHtml);
    });
...
<script>
    ...
    var taskDuration = button.data('task-duration'); // Extract task duration (if needed)

    var modal = $(this);

    // Update modal title and input fields
    modal.find('.modal-title').text('Update Task: ' + taskName);
    modal.find('#task-name').val(taskName || ''); // Set hidden input for task name
    modal.find('#task-duration').val(taskDuration || ''); // Pre-fill duration if available
    });
    // Complete Task button click handler
    $('#complete-task-btn').on('click', function () {
        var updatedDuration = $('#task-duration').val();
        // Perform an AJAX call or other actions to update the task
        console.log('Task Updated with Duration: ' + updatedDuration);
        $('#updateTaskModal').modal('hide'); // Close the modal
    });
</script>
...
<?php /**PATH C:\xampp\htdocs\Employee_attendance\resources\views/home_project.blade.php ENDPATH**/ ?>