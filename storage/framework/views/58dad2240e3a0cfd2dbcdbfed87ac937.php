...
<?php $__env->startSection('content'); ?>
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-1 shadow-none border-radius-xl mt-2 bg-primary ">
        <div class="container-fluid py-1 px-3">
            <?php echo $__env->make('layouts.navbars.auth.topnav', ['title' => 'Home Pre-production'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                                ...
                                <?php if($workOrder->status_code != '110'): ?>
                                    <button class='btn btn-icon btn-2 btn-success mb-0 me-1' type='button' onclick='processClick("<?php echo e($workOrder->workorderid); ?>","<?php echo e($workOrder->status_code); ?>");'>
                                        <span class='btn-inner--icon'><i class='fa fa-edit'></i>
                                            <?php if($workOrder->status_code == '100'): ?> <?php echo 'Assign to User'; ?>
                                            <?php elseif($workOrder->status_code == '101'): ?> <?php echo 'Start Pre-production'; ?>
                                            <?php elseif($workOrder->status_code == '102'): ?>  <?php echo 'Assign to Qc'; ?>
                                            <?php elseif($workOrder->status_code == '111'): ?>  <?php echo 'Check  preproduction upload'; ?>
                                            <?php elseif($workOrder->status_code == '103'): ?>  <?php echo ' Assign to Survey'; ?>
                                            <?php elseif($workOrder->status_code == '112'): ?>  <?php echo 'Restart Pre Production'; ?>
                                            <?php endif; ?>
                                        </span>
                                    </button>    
    ...
    function processClick(workOrderId,status_code)
    {
       ...
       else if (status_code=="101")
       {
         Swal.fire({
                icon: 'warning',
                title: 'WorkOrder',
                text: "Please confirm to start Pre-production",
        ...
        else if (status_code=="102")
       {
            ...
            Swal.fire({
                icon: 'warning',
                title: 'WorkOrder',
                text: "Please confirm to assign QC ",
                showCancelButton: true,
                customClass: {
                    confirmButton: 'btn btn-success mx-2',
                    cancelButton: 'btn btn-danger mx-2'
        ...
        success: function(response) {
            if (response.success) {
					Swal.fire({
						icon: 'success',
						title:'Do you want to complete the Pre-production',
                ...
                Swal.fire({
                    icon: 'error',
                    title: ' No elements found for the preproduction in the database.Please export the elements!',
                    text: response.message
                });
            }
        },
        ...
        { 
			Swal.fire({
                icon: 'warning',
                title: 'WorkOrder',
                text: "Please confirm to restart Pre-production. The latest pre-production file will be downloaded automatically.",
                showCancelButton: true,
                customClass: {
                    confirmButton: 'btn btn-success mx-2',
                    cancelButton: 'btn btn-danger mx-2'
            ...
            success: function(response){
                ...
                if (response.success)
                {
                    Swal.fire({
                                icon: 'success',
                                title: 'WorkOrder',
                                text: "Preproduction started successfully",
                                showCancelButton: false,
                                customClass: {
                                    confirmButton: 'btn btn-success mx-2'
                                },
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\dk3app\resources\views/home_preproduction.blade.php ENDPATH**/ ?>