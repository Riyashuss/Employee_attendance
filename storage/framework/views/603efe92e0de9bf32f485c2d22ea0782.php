...
<div class="ps__rail-x" style="left: 0px; bottom: 0px;"><div class="ps__thumb-x" tabindex="0" style="left: 0px; width: 0px;"></div></div><div class="ps__rail-y" style="top: 0px; right: 0px;"><div class="ps__thumb-y" tabindex="0" style="top: 0px; height: 0px;"></div></div></aside>
        <main class="main-content position-relative border-radius-lg ps">
                <nav class="navbar navbar-main navbar-expand-lg px-0 mx-1 shadow-none border-radius-xl mt-2  ">
...
<div class="card mt-2" id="basic-info" style="background-color:#F2F9FF;width: 95%;margin: auto;">
<div class="card-body mt-1  ">
<div class="card card-custom">
        <h3 class=" text-center mt-2  "> <?php $__currentLoopData = $projectname; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $project): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?> <?php echo e($project->project_name); ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></h3>
        <div id="divNewUserButton" class="row " style="margin-left:20%px;">
                    <div class="col-lg-12 mt-4 ">                
                    <button id="btnAddNewTask" onclick="openAddTaskModal();" class="btn btn-icon5  text-white" style="background-color:#B17457;"style="color:white;" type="button">
                            <span class="btn-inner--icon"><i class="fas fa-plus"></i> </span>
                            <span class="btn-inner--text ms-1">Add  Task</span>
                        </button>  
                    </div>
                </div>
            <div class="table-responsive">
               <table class="table" id="workOrderTable">
                    <thead class=" text-white"style="background-color:#57A6A1;" >
                        <tr>
                    ...
                    <tbody>
                    <?php $__currentLoopData = $taskdetails; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $task): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="mt-2">
                                <td class="text-sm text-center font-weight-normal text-middle"><?php echo e($task->task_name); ?></td>
                                <td class="text-sm text-center font-weight-normal text-middle">
                                    <?php if(!empty($task->username)): ?>
                                        <?php echo e($task->username); ?>

                                    <?php else: ?>
                                        <i class="fas fa-plus"
                                        style="cursor: pointer;color:#8D0B41;"
                                        onclick='showAssignUserModal("<?php echo e($task->id); ?>", "<?php echo e($task->task_name); ?>");'>
                                        </i>
                                    <?php endif; ?>
                                </td>
    ...
    ?>
    <?php echo e($statuses[$task->status] ?? 'New'); ?>

</td>

                             
                                <!-- <td class="text-sm text-center font-weight-normal text-middle"><?php echo e($task->id); ?></td> -->
                                <td class="text-sm text-center font-weight-normal text-middle">
                                <?php if(!empty($task->username)): ?>
                                <button class="btn btn-icon btn-2 mb-0 me-1" style="background-color:#B17457;"
    type="button" onclick='updateproject("<?php echo e($task->id); ?>");'>
    <span class="btn-inner--icon" style="color:white;">
        <i class="fa fa-eye"></i> Update
                                    ...
                                    <button class='btn btn-icon btn-2 mb-0 me-1' type='button' style="background-color:#B17457; color:white;"
    onclick='showAssignUserModal("<?php echo e($task->id); ?>", "<?php echo e($task->task_name); ?>");'>
    <span class='btn-inner--icon' style="color:white;">
        <i class='fa fa-eye'></i> Assign To User
    </span>
</button>
                                           <?php endif; ?>
                                           <button style="background-color:#004A67;" class="btn btn-icon btn-2 btn-info mb-0 me-1" type="button" onclick="edittask(<?php echo e($task->id); ?>)">
    <span class="btn-inner--icon"><i class="fa fa-edit"></i> </span>
</button>

<button class="btn btn-icon btn-2 btn-danger mb-0 me-1" type="button" onclick="updateTaskAction(<?php echo e($task->id); ?>)">
...
<div class="modal fade" id="editTaskModal" tabindex="-1" aria-labelledby="editTaskModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editTaskForm" method="POST" action="<?php echo e(url('/save-task')); ?>">
                    <?php echo csrf_field(); ?>
                    <!-- Task Name Field -->
                    <div class="mb-3">
                        <label for="editTaskName" class="form-label">Task Name</label>
                        <input type="text" class="form-control" id="editTaskName" name="task_name">
                        <input type="hidden" class="form-control" id="editTaskNameid" name="editTaskNameid">
                    </div>
                    <!-- User Selection Dropdown -->
                    <div class="mb-3">
                        <label for="editAssignedUser" class="form-label">Assigned User</label>
                        <select class="form-control" id="editAssignedUser" name="assigned_user_id">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form id="addTaskForm" name="addTaskForm"> <!-- Add ID & Name -->
                <?php echo csrf_field(); ?>
                <input type="hidden" name="_token" value="<?php echo e(csrf_token()); ?>"> <!-- Fix CSRF -->
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <?php $__currentLoopData = $projectname; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $project): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?> 
                            ...
                            <input type="text" id="project_name" class="form-control" value="<?php echo e($project->project_name); ?>" readonly>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="user_name">Assign To</label>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-secondary" onclick="$('#addTaskModal').modal('hide')">Cancel</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" id="assignUserModal" tabindex="-1" aria-labelledby="assignUserModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="assignUserModalLabel">Assign Task to User</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                <button type="button" class="btn btn-success" onclick="assignToUser();">Assign</button>
            </div>
        </div>
    </div>
</div>
            ...
            <div class="modal-body">
                <form id="updateTaskForm">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control" id="assignedUser" name="assignedToUserId">
                            <option value="">Select User</option>
                            <?php $__currentLoopData = $userdetails; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($user->id); ?>"><?php echo e($user->username); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<?php $__env->startPush('js'); ?>
    ...
    $('#addTaskModal').modal('show'); // Show the modal using Bootstrap
}

function updateTaskAction(taskId) {
    Swal.fire({
        title: 'Are you sure?',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '<?php echo e(route('update-task-action')); ?>', // Ensure the route is correct
                type: 'POST',
                data: {
                    task_id: taskId,
    ...
    function updateproject(task_id) {
        console.log(task_id)
        window.location.href = '/updateproject/' +task_id;
    }
    function updateuserproject(task_id) {
        console.log(task_id)
        window.location.href = '/updateuserproject/' +task_id;
    }
    ...
    $('#addTaskForm').on('submit', function (e) {  // Correctly target the form
...
function edittask(id) {
    // Fetch task details by task id using AJAX
    $.ajax({
        url: '/task/details/' + id, // Replace task_id with id
        type: 'GET',
        success: function(response) {
            // Assuming the response contains the task name and the assigned user ID
            $('#editTaskName').val(response.task_name); // Populate task name
            $('#editAssignedUser').val(response.user_id); // Populate assigned user
            $('#editTaskNameid').val(id);
...
function saveTaskChanges() {
    ...
    var userid = $('#editAssignedUser').val();
    var taskid = $('#editTaskNameid').val();

    // Validate if both fields are filled
    if (!taskname || !userid) {
        Swal.fire({
            title: 'Validation Error!',
            text: 'Please provide both task name and assigned user.',
            icon: 'warning',
            confirmButtonText: 'OK'
        });
        return;
    ...
    formData.append('editTaskNameid', taskid);
    ...
    fetch('/save-task', {
        ...
        body: formData,
        headers: {
            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' // Add CSRF token here
        }
    })
        ...
        .then(data => {
            if (data.success) {
                Swal.fire({
                    ...
                    // Optionally reload the table or update the UI here
...
function assignUserToTask() {
    const taskId = document.getElementById('assignTaskForm').getAttribute('data-task-id');
    ...
    // Make an AJAX call to update the task assignment
...
//     window.location.href = "/taskupdate/" + taskId;
//     }
function assignToUser()
    {
       
        var assignedToUserId = $('#assignedUser').val();
        var assignedToUserName = $('#assignedUser option:selected').text();
        ...
        {
            $('#assignedUser').html('Please select username');
            $('#assignedUser').show();
            return;
        }
...
function showAssignUserModal(taskId, taskName) {
    // Populate modal fields
    document.getElementById('taskName').value = taskName;
    document.getElementById('assignedUser').value = ""; // Reset the dropdown
    // Store taskId for submission
    document.getElementById('assignTaskForm').setAttribute('data-task-id', taskId);
...
function saveTaskDetails() {
    ...
    const assignedToUserId = document.getElementById('assignedUser').value;
...
</html><?php /**PATH C:\xampp\htdocs\Nilatech_attendance\resources\views/laravel/project/view_project.blade.php ENDPATH**/ ?>