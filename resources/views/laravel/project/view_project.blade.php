...
<div class="card card-custom">
        <h3 class=" text-center mt-2  "> @foreach ($projectname as $project) {{ $project->project_name }}@endforeach</h3>
            ...
            <div class="table-responsive">
               <table class="table" id="workOrderTable">
                    ...
                    @foreach ($taskdetails as $task)
                            <tr class="mt-2">
                                <td class="text-sm text-center font-weight-normal text-middle">{{ $task->task_name }}</td>
                                <td class="text-sm text-center font-weight-normal text-middle">
                                    ...
                                    @else
                                        ...
                                        onclick='showAssignUserModal("{{ $task->id }}", "{{ $task->task_name }}");'>
                                        </i>
                                    @endif
                                </td>
    ...
    @endphp
    {{ $statuses[$task->status] ?? 'New' }}
</td>

                             
                                <!-- <td class="text-sm text-center font-weight-normal text-middle">{{ $task->id }}</td> -->
                                <td class="text-sm text-center font-weight-normal text-middle">
                                @if(!empty($task->username))
                                <button class="btn btn-icon btn-2 mb-0 me-1" style="background-color:#B17457;"
    type="button" onclick='updateproject("{{ $task->id }}");'>
    ...
    <span class='btn-inner--icon' style="color:white;">
        <i class='fa fa-eye'></i> Assign To User
                                           ...
                                           @endif
                                           <button style="background-color:#004A67;" class="btn btn-icon btn-2 btn-info mb-0 me-1" type="button" onclick="edittask({{ $task->id }})">
    <span class="btn-inner--icon"><i class="fa fa-edit"></i> </span>
</button>

<button class="btn btn-icon btn-2 btn-danger mb-0 me-1" type="button" onclick="updateTaskAction({{ $task->id }})">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editTaskForm" method="POST" action="{{ url('/save-task') }}">
                    @csrf
                    <!-- Task Name Field -->
                    <div class="mb-3">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form id="addTaskForm" name="addTaskForm"> <!-- Add ID & Name -->
                @csrf
                <input type="hidden" name="_token" value="{{ csrf_token() }}"> <!-- Fix CSRF -->
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        @foreach ($projectname as $project) 
                            ...
                            <input type="text" id="project_name" class="form-control" value="{{ $project->project_name }}" readonly>
                        @endforeach
                    </div>
                    <div class="form-group">
...
function updateTaskAction(taskId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '{{ route('update-task-action') }}', // Ensure the route is correct
                type: 'POST',
                data: {
                    task_id: taskId,
...
icon: 'warning',
title: '@lang('Checkin')',
text: '@lang('Please Select the Location')',
...
if(parseInt(assignedToUserId) === 0)
{
$('#user_name_error').html('@lang('unsnbemt')');
...
fd.append('_token','{{csrf_token()}}');
$.ajax({
url: '{{route('assignToUserPreproductionPost')}}',
type: 'post',
data:fd,
contentType: false,
processData: false,
...
icon: 'success',
title: '@lang('words.workorder')',
text: '@lang('words.wororderassign')' + assignedToUserName,
showCancelButton: false,
customClass: {
confirmButton: 'btn btn-success mx-2'
},
