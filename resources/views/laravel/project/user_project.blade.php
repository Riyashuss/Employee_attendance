...
@foreach($groupedProjects as $projectName => $users)
    <div class="project-section p-3 bg-white text-dark" style="border-radius: 10px;">
        ...
        @foreach($users as $user)
            @if ($user->leave_id == 1 && Carbon::parse($user->leave_date)->isToday()) <!-- Leave today (absent) -->
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div class="d-flex align-items-center gap-2 w-50">
                        <p><strong></strong> {{ $user->username ?? 'No user assigned' }}</p>
                    </div>
                    <div>
                        <button type="button"
                    ...
                    <!-- Modal for each user -->
            <div class="modal fade" id="viewModal-{{ $user->user_id }}" tabindex="-1" aria-labelledby="viewModalLabel-{{ $user->user_id }}" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="viewModalLabel-{{ $user->user_id }}">User Details</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <!-- <p><strong>User ID:</strong> {{ $user->user_id ?? 'No ID assigned' }}</p> -->
                            <p><strong>Username:</strong> {{ $user->username ?? 'No user assigned' }}</p>
                            <p><strong>Present Count (This Month):</strong> {{ $userStats[$user->user_id]->present_count ?? 0 }}</p>
                            <p><strong>Absent Count (This Month):</strong> {{ $userStats[$user->user_id]->absent_count ?? 0 }}</p>
                            <p><strong>Total Working Days (This Month):</strong> {{ $workingDays }}</p>
                        </div>
                        <div class="modal-footer">
        ...
        @foreach($attendance as $user)
          @if ($user->leave_id == 0 && Carbon::parse($user->leave_date)->isToday())
            <div class="d-flex justify-content-between align-items-center mb-3">
                <div class="d-flex align-items-center gap-2 w-50">
                    <p>
                        {{-- <strong >User ID:</strong> {{ $user->user_id ?? 'No ID assigned' }} <br> --}}
                        <strong></strong> {{ $user->username ?? 'No user assigned' }}
                    </p>
                </div>
                <div>
                    <button type="button"
        ...
        @foreach($allUsers as $allUser)
            ...
            @if (!$isAbsent && !$isPresent)
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div class="d-flex align-items-center gap-2 w-50">
                        <p>
                            {{-- <strong>User ID:</strong> {{ $allUser->id ?? 'No ID assigned' }} <br> --}}
                            <strong></strong> {{ $allUser->username ?? 'No user assigned' }}
                        </p>
                    </div>
                    <div>
                        <button type="button"
                            class="btn btn-danger"
                            data-bs-toggle="modal"
                            data-bs-target="#viewModal-{{ $allUser->id }}">
                        View
                    </button>
                    </div>
                </div>
                 <!-- Modal for each user -->
            <div class="modal fade" id="viewModal-{{ $allUser->id }}" tabindex="-1" aria-labelledby="viewModalLabel-{{ $allUser->id }}" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="viewModalLabel-{{ $allUser->id }}">User Details</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <!-- <p><strong>User ID:</strong> {{ $allUser->id ?? 'No ID assigned' }}</p> -->
                            <p><strong>Username:</strong> {{ $allUser->username ?? 'No user assigned' }}</p>
                            <p><strong>Present Count (This Month):</strong> {{ $userStats[$allUser->id]->present_count ?? 0 }}</p>
                            <p><strong>Absent Count (This Month):</strong> {{ $userStats[$allUser->id]->absent_count ?? 0 }}</p>
