...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/fullcalendar/3.9.0/fullcalendar.css" />
  ...
  @media (max-width: 480px) {
    .sidenav {
      position: fixed;
      overflow-x:visible;
      width: 129%; /* Full width for small screens */
    ...
    <a class="navbar-brand"style="backgrounds-color:white" href="">
        ...
        <ul class="navbar-nav">
            ...
            <li class="nav-item" id="ddd">
                <a href="{{ route('home_user') }}" class="nav-link " aria-controls="dashboard" role="button" aria-expanded="false">
                    <div class="icon icon-shape icon-sm text-center d-flex align-items-center justify-content-center">
                        <!-- <i class="fa fa-house text-sm opacity-10" style="color: #FD1717;"></i> -->
                    </div>
            ...
            <li class="nav-item sidemenuitem" id="menuAttendance" style="">
                <a href="{{ route('home_attendance') }} " class="nav-link " aria-controls="attendance" role="button" aria-expanded="false">
                    <div class="icon icon-shape icon-sm text-center d-flex align-items-end justify-content-center">
                        <!-- <i class="fas fa-history text-sm opacity-10" style="color: #00FF13;"></i> -->
                    </div>
                    <span class="nav-link-text ms-1" style="color:white">Attendance</span>
                </a>
            </li>
            <li class="nav-item sidemenuitem" id="menuProject" style="">
                <a href="{{ route('home_Project') }} " class="nav-link " aria-controls="Project" role="button" aria-expanded="false">
                    <div class="icon icon-shape icon-sm text-center d-flex align-items-end justify-content-center">
                        <!-- <i class="fa-solid fa-dice opacity-10" style="color: #00FF13;"></i> -->
                    </div>
                    <span class="nav-link-text ms-1" style="color:white">Project</span>
                </a>
            </li>

            <li class="nav-item sidemenuitem" id="details" style="display:none;">
                <a data-bs-toggle="collapse" href="#workorders" class="nav-link " aria-controls="details" role="button" aria-expanded="false">
                    <div class="icon icon-shape icon-sm text-center d-flex align-items-end justify-content-center">
                ...
                <div class="collapse show" id="details">
                    <ul class="nav ms-4">
                        <li class="nav-item ">
                            <a class="nav-link " href="{{ route('projectview') }}">
                                <div class="icon icon-shape icon-sm text-center d-flex align-items-end justify-content-center">
                                    <!-- <i class="fa fa-file-pen text-sm opacity-10" style="color: #00FFFF;"></i> -->
                                </div>
...
{{-- <div class="modal fade" id="confirmationModal" tabindex="-1" aria-labelledby="confirmationModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p id="modalMessage"></p> <!-- This is where the message will be displayed -->
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                <button type="button" id="confirmLeaveBtn" class="btn btn-primary">OK</button>
            </div>
        </div>
    </div>
</div> --}}
       <div class="modal fade" id="confirmationModal" tabindex="-1" aria-labelledby="confirmationModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" id="okLeaveBtn" data-bs-dismiss="modal">Ok</button>
                        <button id="deleteLeaveBtn" type="button" class="btn btn-danger">Delete</button>
                        <button type="button" id="confirmLeaveBtn" class="btn btn-primary">Confirm</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="card" id="basic-info" style="width: 70%;margin: auto;">
    <h3 type="text-center mt-2 " style="margin:auto;margin-top:20px;">Welcome  <?php echo auth()->user()->username; ?></h3>
  
    <div class="card-body mt-5 .main "style="height:60%;">
        <form action="leave" method="POST">
            <!-- <div class="container"> -->
                <div id='calendar'></div>
                {{-- <button type="button" class="btn" id="submitLeaveBtn">Leave</button> --}}
            <!-- </div> -->
        </form>
        ...
        <script>
                ...
                editable: true,
                events: SITEURL + "/fetch-leaves",
                displayEventTime: false,
                selectable: true,
                selectHelper: true,
                        
    // ✅ Fix for Mobile: Use dayClick to handle selection
...
if (selectedDateObject < today && selectedDate !== todayFormatted)
{
                            // Fetch attendance record for the past date
                            $.ajax({
                                url: SITEURL + "/attendance/" + selectedDate, // URL for fetching attendance record
                                type: "GET",
                                success: function (response) {
                                    if (response.success) {
                                        // Show attendance record in the modal
                                        var attendanceMessage = " SelectDate:" + selectedDate    ;
                                        attendanceMessage += "       Check-in-Time:  "+ response.data.checkin+"   And";
                                        attendanceMessage += "       Check-out-Time: " + response.data.checkout+  " And";
                                        attendanceMessage += "       location_name: " + response.data.location_name;
                                        showModal(attendanceMessage, false, "OK");
                                    } else {
                                        // Show the message if no attendance record is found
                                        showModal(response.message, false, "OK");
                                    }
                                },
                                error: function (xhr) {
                                    // Show error if there's an issue with the request
                                    showModal("An error occurred while fetching attendance: " + xhr.responseText, false, "OK");
                                }
                            });

                            // Unselect the past date after showing the attendance record
                            calendar.fullCalendar('unselect');
                            return;
                        }

                        // Check if the user has already taken leave on the selected date
                        var leaveExists = false;
                        var events = calendar.fullCalendar('clientEvents');
                      
                        events.forEach(function (event) {
                      
                            if (event.start.format("YYYY-MM-DD") === selectedDate && event.title === "Leave") {
                        ...
                        // if (leaveExists) {
                        //     // Show modal with "OK" and "Delete" buttons if leave already exists
                        //     showModal("You have already taken leave on this date.", false, "ok");
                        //     calendar.fullCalendar('unselect'); // Unselect the date if already taken
                        //     return;
                        // }
                        if (leaveExists) {
                              // Change Leave Button Color to Red
                            $("#confirmLeaveBtn").css({
                            ...
                            // Show modal with "OK" and "Delete" buttons if leave already exists
                            showModal("You have already taken leave on this datesss.", false, "ok");
                            calendar.fullCalendar('unselect'); // Unselect the date if already taken
                            return;
                        } else {
                            // Reset Leave Button Color to Default if no leave exists
                            $("#confirmLeaveBtn").css({
                                'background-color': '',  // Reset to default
                                'color': ''
                            });
                        }
                        // Future date - Show confirmation modal with "Confirm" and "Cancel" buttons
                        showModal("Are you sure you want to take leave on " + selectedDate + "?", true, "Leave");
                    }
                });


                function showModal(message, showButtons, confirmText) {
                    $("#modalMessage").text(message);
                 
             
                    if (showButtons) {
                        $("#confirmLeaveBtn").show();
                        $("#confirmLeaveBtn").text(confirmText);
                        $("#cancelLeaveBtn").show();
                        $("#deleteLeaveBtn").hide(); // Show the "Delete" button
                    }else if (confirmText=="ok") {
                        $("#confirmLeaveBtn").hide();
                        // $("#confirmLeaveBtn").text(confirmText);
                        $("#cancelLeaveBtn").show();
                        $("#deleteLeaveBtn").show();
                        ...
                        $("#confirmLeaveBtn").hide();
                        $("#cancelLeaveBtn").hide();
                        $("#deleteLeaveBtn").hide(); // Hide the "Delete" button
                    }
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response && response.message) {
                                ...
                                showModal("Unexpected response format.", false, "OK");
                            }
                        },
                        error: function (xhr) {
                            showModal("An error occurred: " + xhr.responseText, false, "OK");
                        }
                    });
                });

                // Delete Leave action
                $("#deleteLeaveBtn").on("click", function () {
                    $("#confirmationModal").modal("hide");

                    $.ajax({
                        url: SITEURL + "/delete-leave",
                        type: "POST",
                        data: {
                            user_id: {{ Auth::id() }},
                            date: selectedDate
                        ...
                        success: function (response) {
                            if (response && response.success) {
                                showModal("Leave successfully deleted.", false, "OK");
                                calendar.fullCalendar('removeEvents', function (event) {

                                    return event.start.format("YYYY-MM-DD") === selectedDate && event.title === "Leave";
                                });
                            } else {
                                showModal("Failed to delete leave: " + response.message, false, "OK");
                            }
                        },
                        error: function (xhr) {
                            showModal("An error occurred while deleting leave: " + xhr.responseText, false, "OK");
                        }
                    });
                });

                // Cancel action for modal
                $("#cancelLeaveBtn").on("click", function () {
                    $("#confirmationModal").modal("hide");
                    calendar.fullCalendar('unselect');
...
<style>
    ...
    .fc-day-leave {
        background-color:rgb(0, 0, 0) !important; /* Yellow background for leave days */
